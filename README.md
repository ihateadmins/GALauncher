...
C# .NET Framework (compatible with Windows and Linux/Wine!)
...
### Requirements
- Windows or Linux (with Wine installed for Linux compatibility).
...
- Run as Administrator on Windows (launcher will prompt elevation if needed).
...
1. **Join the community**:
   - Discord: https://discord.gg/EeHwBpCPsK (main hub for server status, registration for new server!, updates, and help).
...
3. **Place the launcher**:
   - Download and extract `GALauncher2025.exe` (and optional resources like `app.ico`, `menubg.jpg`) anywhere convenient.

4. **Run the launcher**:
   - Double-click `GALauncher2025.exe` (or run via Wine on Linux: `wine GALauncher2025.exe`).
   - It will prompt for admin rights on Windows if necessary.
   - Configure:
